...
namespace vdff {
  ...
  Tensor3f::Tensor3f(Utils::InfoImgSeq imgInfo, std::string name) 
...
#else
{
  ...
  Tensor3f::Tensor3f(cv::Mat mat, std::string name)
  {
    ...
    init();
    allocHost();
    openCVHelpers::convert_mat_to_layered(h_img,mat);
  }
  ...
  void Tensor3f::setImageInSequence(int imgIndex, const cv::Mat slice){
    ...
    openCVHelpers::convert_mat_to_layered(h_img_layerPtr, slice);
  }
  ...
  cv::Mat Tensor3f::getImageInSequence(int imgIndex){
    ...
    openCVHelpers::convert_layered_to_mat(m_img, h_img_layerPtr);
    return m_img;
  }
}