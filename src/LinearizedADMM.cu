...
cv::Mat LinearizedADMM::run(float *d_energyDerivative, size_t derivativeDegree, float dataFidelityParam, float tau,
  ...
  convert_mat_to_layered(l_u, init);
  ...
  for (size_t it = 0; it < maxIter; ++it) {
    ...
    if (plotIterations /*&& it % 50 == 0*/) {
      ...
      convert_layered_to_mat(vis, l_u);
      
      //char buf[256];
      //sprintf(buf, "Iteration %zd", it);
      showDepthImage("Current Iteration", vis, 100 + 2*(init.cols + 80), 500);      
      
      char key = waitKey2(1,false);
  ...
  cudaMemcpy(l_u, d_u, nrBytes, cudaMemcpyDeviceToHost); CUDA_CHECK;
  convert_layered_to_mat(res, l_u);
  
  // --- free CUDA memory
  freeCUDAMemory();

  // print stats for linearized admm
  cout << "Average time for updating U: " << getAverage(timeUpdateU) << " s" << endl;
  cout << "Average time for thresholding U: " << getAverage(timeThresholdU) << " s" << endl;
  cout << "Average time for calculating Ux and Uy: " << getAverage(timeCalcUxUy) << " s" << endl;
  cout << "Average time for updating G: " << getAverage(timeUpdateG) << " s" << endl;
  cout << "Average time for updating B: " << getAverage(timeUpdateB) << " s" << endl;
  cout << "Average time for one iteration of ADMM: " << getAverage(timeOneIteration) << " s" << endl;
